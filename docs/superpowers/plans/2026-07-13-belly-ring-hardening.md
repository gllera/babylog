...
**Review note (code-quality pass, addressed in a follow-up comment commit):** the backtest objective in `fgForecastErr` is bimodal around a `ref≈0` cliff. When a candidate half-life's span is shorter than the baby's median feed gap, her pre-feed fullness under that candidate reads ~0, so its day/night reference rounds to 0, no forecast can cross below 0, and every interval scores the 12h miss-bound. The effect: the pick is effectively the shortest grid t½ whose span clears her typical gap, then split on forecast quality — and with realistic gap *jitter* the 60-min prior is not strictly pinned for equal-size schedules (it can be nudged to ~75 when its own span stops clearing the gap). Also, frequent feeding biases the pick toward the shorter/safer anchor. Both are prior-leaning and touch only the countdown *slope* — never fullness (fullnessAt/digSumMl self-calibrate to the same k0, so `fullness + digested = fed` still holds exactly), and recovery is good on realistic on-demand data where pre-feed fullness is a consistent positive level. **Deferred, not fixed:** sharpening the objective past the cliff (e.g. `f <= ref`, or dropping candidates whose reference rounds to ~0 so the score reflects forecast quality rather than a rounding artifact) is a modeling change needing its own analysis and fixtures — the `ref≈0` for a too-fast candidate is arguably *correct* signal ("this t½ implies she's fed while empty, which is implausible"), so it warrants brainstorming, not a reflexive patch. The comment on the calibration block records this behavior honestly.
